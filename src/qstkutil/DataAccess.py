...
class DataAccess(object):
    ...
    def get_data_hardread(self, ts_list, symbol_list, data_item, verbose=False, bIncDelist=False):
        ...
        for symbol_ctr, symbol in enumerate(symbol_list):
            _file = None
            ...
            try:
                ...
                if bIncDelist:
                    ...
                    if file_path is None and len(lsDelPaths) > 0:
                        print 'Found delisted paths:', lsDelPaths

                ''' If we don't have a file path continue... unless we have delisted paths '''
                if (not isinstance(file_path, type("random string"))):
                    if not bIncDelist or not lsDelPaths:
                        continue
                        # File not found
            ...
            assert(not _file is None or bIncDelist)
            ...
            if bIncDelist and len(lsDelPaths) and naData is None:
    ...
    def getPathOfFile(self, symbol_name, bDelisted=False):
        ...
        if not bDelisted:
            ...
            for sPath in self.folderList:
                if re.search('Delisted Securities', sPath) is None:
                    continue

                for sFile in dircache.listdir(sPath):
                    if not re.match('%s-\d*.pkl' % symbol_name, sFile) is None:
    ...
    def get_symbols_from_list(self, s_list):
        ...
        for path in self.folderList:
            ...
            with open(path_to_look, 'r') as f:
                for line in f:
                    ls_symbols.append(line[:-1])
