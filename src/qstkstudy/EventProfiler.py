...
class EventProfiler(object):
    ...
    def __init__(self, eventMatrix, startday, endday,
        ...
        self.close = self.dataobj.get_data(
            self.timestamps,
    ...
    def study(self, filename, method="mean",
        ...
        for col in range(0, events.shape[1]):
            ...
            for row in range(0, events.shape[0]):
                ...
                if events[row, col] == 1.0:
                    ...
                    impact[:, currcol] = rets[row - self.lookback_days: row + self.lookforward_days + 1, col]
                    currcol = currcol + 1
        ...
        plt.axhline(
            y=1.0, xmin=-self.lookback_days, xmax=self.lookforward_days + 1,
            color='#000000')
        if plotErrorBars:  # draw errorbars if user wants them
            plt.errorbar(
                studyrange[self.lookback_days:],
                studystat[self.lookback_days:],
                yerr=studystd[self.lookback_days:],
                ecolor='#AAAAFF',
                alpha=0.1)
        plt.plot(
            studyrange, studystat, color='#0000FF', linewidth=3,
            label='mean')
        ...
        if plotMarketNeutral:
            plt.title('market relative mean of %d events' % int(numevents))
        else:
            plt.title('mean of %d events' % int(numevents))
        plt.xlabel('Days')
