...
def _calculate_leverage(values_by_stock, ts_leverage):
    ...
    for r_index, r_val in values_by_stock.iterrows():
        f_long = sum(val for val in r_val.values[:-1] if val > 0)
        f_short = sum(val for val in r_val.values[:-1] if val < 0)
...
def tradesim(alloc, df_historic, f_start_cash, i_leastcount=1,
             ...
             i_target_leverage=1, f_rate_borrow=0.0, log=""):
    ...
    for row_index, row in alloc.iterrows():
        ...
        prediction_price = df_historic.ix[df_historic.index[pred_index]:].ix[0:1]
        ...
        if b_first_iter:
            ...
            to_calculate = df_historic[
                (df_historic.index <= prediction_date) &
        ...
        same = all(str(cash_delta_less_shares[sym].values[0]) == str(no_trans_shares[sym].values[0])
                   for sym in shares)
        ...
        if not same:
            ...
            order = pand.Series(((prediction_shares.values - shares.values)[0])[:-1], index=row.index[:-1])
            ...
            if not np.isnan(f_slippage_cost):
                f_total_slippage += f_slippage_cost
            ...
            for sym in shares:
                if sym != "_CASH":
                    ...
                    if log:
                        if(abs(order[sym]) != 0):
                            columns = [
                                sym, sym, order_type, prediction_date, GL, order[sym],
                                trade_price[sym].values[0],
                                trade_price[sym].values[0] * order[sym],
                                shares[sym].ix[-1], value_after_trade, f_stock_commission, round(f_slippage_cost, 2)
                            ]
