...
def quickSim(alloc, historic, start_cash):
    ...
    msg = 'Please use qstksim.tradesim instead, or comment me out in %s, line %i' % \
    ...
    for row_index, row in alloc.iterrows():
        ...
        to_calculate = historic[
            (historic.index <= trade_date) &
...
def _compute_short(arr):
    ...
    #        tally = tally + arr[i]
    #return abs(tally)
    # I don't know why this omits the last item
    return sum(a for a in arr[:-1] if a < 0)
...
def _compute_leverage(arr, fundval):
    ...
    return fundval or (_compute_long(arr) - _compute_short(arr)) / fundval
...
def shortingQuickSim(alloc, historic, start_cash, leverage):
    ...
    cash_values = pand.DataMatrix([shares * closest.values[-1, :]], index=[closest.index[-1]])
    ...
    for i in range(1, len(alloc.values[:, 0])):
        ...
        for date in closest[closest.index > fund_ts.index[-1]].index:
            ...
            fund_ts = fund_ts.append(pand.Series([(closest.xs(date) * shares).sum()], index=[date]))
            cash_values = cash_values.append(pand.DataMatrix([shares * closest.xs(date)], index=[date]))
    ...
    for date in historic[historic.index > alloc.index[-1]].index:
        if date in closest.index:
            ps = pand.Series([(closest.xs(date) * shares).sum()], index=[date])
            fund_ts = fund_ts.append(ps)
