...
def query(allQueries, k, method='mean'):
    ...
    omitLastCol = data.shape[1] - 1
    # It must have two columns at least. Possibly add a check for this?
    ...
    for ctr in range(0, limit):  # for every query point...
        ...
        if (method == 'median'):
            ...
            # if median ends
        if (method == 'mode'):
            answer[ctr] = sc.stats.mode(data[index[0:k], -1])[0][0]
            # The first mode. If there is more than one mode then the only the first mode is returned
            #endif mode
